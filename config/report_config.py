...
SELECT   	  tt1.tag_id, 
         ...
         IFNULL( t1.spent_cpm,0) AS spent,
  
	  IFNULL(t3.payout,0) AS rev_opt, 
	  IFNULL(t3.payout,0) AS rev_real, 
...
SELECT tag_id, DATE,
                       ...
                       SUM(paid) AS paid,               
                       SUM(ctr) AS ctr
   FROM stats_tags_date_by_node_hour WHERE DATE={date}
...
(SELECT tag_id, DATE, SUM(imp) AS imp,
   ...
   FROM stats_ads_tags_date_by_node_hour WHERE DATE={date}
   
   GROUP BY tag_id, DATE)  t1 ON  tt1.tag_id = t1.tag_id AND tt1.date = t1.date
LEFT JOIN
  (
  SELECT tag_id,
          SUM(payout) AS payout,
          SUM(conversions) AS conversions, DATE
   FROM conversions_click
   WHERE DATE={date}
   GROUP BY tag_id) t3 ON tt1.tag_id=t3.tag_id
AND tt1.date=t3.date
    """,
        'index': ['tag_id', 'date'],
...
SELECT tag_id, 
   ...
   WHERE DATE={date}
   GROUP BY tag_id,device1, device2
)
 tt1 LEFT JOIN
(SELECT tag_id,
	device1,
	...
	SUM(spent_cpc_raw) AS spent_cpc_raw,
	SUM(spent_cpc_landing) AS spent_cpc_landing
FROM stats_ads_tags_device_by_node_hour WHERE DATE={date}
...
LEFT JOIN
  ...
  SELECT tag_id,
   ...
   WHERE DATE={date}
   GROUP BY tag_id
   ) t3 ON tt1.tag_id=t3.tag_id
AND tt1.date=t3.date
AND tt1.device1 = t3.device1
AND tt1.device2 = t3.device2
    """,
...
SELECT tt1.tag_id,
       ...
       t1.click_raw,
       t1.click_landing,
       t1.click_raw AS click,
       t1.spent_cpm,
       t1.spent_cpc_raw,
       t1.spent_cpc_landing,
       t1.spent_cpm AS spent,
       IFNULL(t3.payout, 0) AS rev_opt,
       IFNULL(t3.payout, 0) AS rev_real
FROM
  (SELECT tag_id, DATE, location1,
                        SUM(request) AS request,
                        SUM(paid) AS paid
...
(SELECT tag_id,
	...
	SUM(spent_cpc_raw) AS spent_cpc_raw,
	SUM(spent_cpc_landing) AS spent_cpc_landing
FROM stats_ads_tags_browser_by_node_hour WHERE DATE={date}
...
FROM
  (SELECT tag_id, 
   ...
   GROUP BY tag_id,location1,device1,device2) t1
...
SELECT t1.tag_id,
       ...
       IFNULL(t3.payout,0) AS rev_opt,
       IFNULL(t3.payout,0) AS rev_real,
       IFNULL(t3.payout,0) AS conversions
FROM
  (SELECT tag_id,
